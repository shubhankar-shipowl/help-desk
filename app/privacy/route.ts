...
// 1. Facebook OAuth callbacks (when Facebook redirects with code/state/error)
// 2. Facebook webhook verification (when Facebook sends hub.mode=subscribe)
export async function GET(req: NextRequest) {
  try {
    ...
    if (hubMode === 'subscribe') {
      ...
      console.log('[Facebook Webhook Verification] 📥 Incoming verification request at /privacy')
    ...
    // If neither webhook verification nor OAuth callback, this is a normal page visit
    // Return 404 to let Next.js render the page.tsx component
...
// Handle POST requests for webhook events (Facebook sends webhook data via POST)
export async function POST(req: NextRequest) {
  // Forward webhook POST requests to the actual webhook handler at /webhooks/facebook
  // We need to construct the full URL and forward the request
  try {
