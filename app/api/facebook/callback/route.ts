...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log('Exchanging code for access token...', {
      hasCode: !!code,
      redirectUri,
      appId: appId?.substring(0, 4) + '...', // Only log first 4 chars for security
    })
    ...
    for (const page of pages) {
      ...
      // Note: This might fail if permissions are missing, but we continue anyway
      try {
        ...
        if (subscribeResponse.ok) {
          ...
          console.warn(`[Facebook Callback] 💡 Note: 'mention' field may require App Review or may not be visible in UI`)
          console.warn(`[Facebook Callback] 💡 You can subscribe via Graph API: curl -X POST "https://graph.facebook.com/v18.0/${page.id}/subscribed_apps?subscribed_fields=mention&access_token=YOUR_TOKEN"`)
          console.warn(`[Facebook Callback] 💡 You can subscribe manually via Facebook Developer Console or run: npx tsx scripts/subscribe-facebook-feed.ts`)
          // Continue even if subscription fails - user can do it manually via UI or script
        }
      } catch (subscribeError: any) {
        console.warn(`[Facebook Callback] ⚠️ Error attempting to subscribe page ${page.id} to feed:`, subscribeError.message)
        console.warn(`[Facebook Callback] 💡 You can subscribe manually via Facebook Developer Console or run: npx tsx scripts/subscribe-facebook-feed.ts`)
        // Continue even if subscription fails - user can do it manually
      }
    }
