 ...
 * - Development (NODE_ENV=development): http://localhost:4002
...
export function maskPhoneNumbersInText(text: string | null | undefined): string {
  ...
  // If the text looks like HTML, only mask phone numbers in visible text,
  // leaving HTML attributes (src, href, style, etc.) untouched.
  if (text.includes('<') && text.includes('>')) {
    return maskPhoneNumbersInHtml(text)
  }

  return applyPhoneMasking(text)
}

/**
 * Mask phone numbers only in the text portions of HTML,
 * preserving all HTML tags and their attributes intact.
 */
function maskPhoneNumbersInHtml(html: string): string {
  // Split HTML into tags and text segments
  // This regex matches HTML tags (including comments, scripts, styles)
  const parts = html.split(/(<[^>]*>)/g)

  for (let i = 0; i < parts.length; i++) {
    const part = parts[i]
    // Skip HTML tags (they start with <) and empty strings
    if (!part || part.startsWith('<')) continue
    // Apply phone masking only to text content
    parts[i] = applyPhoneMasking(part)
  }
...
/**
 * Apply phone number masking to plain text (no HTML).
 */
function applyPhoneMasking(text: string): string {
  const phonePatterns = [
  ...
  phonePatterns.forEach((pattern) => {
    maskedText = maskedText.replace(pattern, (match) => {
      ...
      if (allDigits.length >= 10) {
        ...
        if (match.includes('-')) {
          return allDigits.length === 10 ? `XXX-XXX-${last4}` : `XXX-${last4}`
