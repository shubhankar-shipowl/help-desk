...
// Defer PrismaAdapter and prisma imports to avoid build-time failures.
// These are loaded lazily so the module can be safely imported during next build
  ...
  return {
    ...
    callbacks: {
      ...
      async redirect({ url, baseUrl }) {
        // Use NEXTAUTH_URL as the canonical public-facing URL, fall back to baseUrl
        const actualBaseUrl = (process.env.NEXTAUTH_URL || process.env.APP_URL || baseUrl).replace(/\/$/, '');
        ...
        // If url is absolute, allow it if it matches the base origin
        try {
