...
// Suppress known megajs internal errors (uploads still succeed)
...
const megaErrorHandler = (reason: any, _promise?: Promise<any>) => {
  ...
  // ENOENT (-9) occurs when a MEGA file/node no longer exists (e.g., after deletion)
  // This is expected when emails referencing old MEGA files are still cached/loaded
  const isEnoentError =
...
function getFolderForFileType(mimeType: string): string {
  if (mimeType.startsWith('image/')) {
    ...
    return 'app-backups/help-desk/videos'
  } else {
