...
export function extractInlineImagesFromHtml(
  ...
  parsedAttachments?: Array<{ cid?: string; contentType?: string; content?: Buffer; filename?: string }>
): InlineImage[] {
  if (!htmlContent) {
  ...
  console.log('   🔍 [Extract Images] Scanning HTML for inline images/videos...')
  ...
  if (htmlContent.length > MAX_HTML_SIZE) {
    console.warn(`   ⚠️  [Extract Images] HTML too large (${(htmlContent.length / 1024 / 1024).toFixed(2)} MB), skipping inline image extraction to avoid stack overflow`)
    return []
  }
  ...
  try {
    ...
    console.log(`   - Found ${tagPositions.length} potential image/video tag(s) in HTML`)
    
    // Now extract src from each tag individually (more memory efficient)
    for (let i = 0; i < tagPositions.length; i++) {
      const { index, tagType } = tagPositions[i]
      
      // Extract a reasonable chunk around the tag (max 10KB per tag)
      const chunkStart = Math.max(0, index)
      const chunkEnd = Math.min(htmlContent.length, index + 10000)
      const chunk = htmlContent.substring(chunkStart, chunkEnd)
      
      // Find the src attribute in this chunk
      ...
      if (srcMatch && srcMatch[1]) {
        ...
        if (src.startsWith('data:') || src.startsWith('cid:')) {
          console.log(`   [${i + 1}/${tagPositions.length}] Found ${tagType} tag with src: ${src.substring(0, 80)}${src.length > 80 ? '...' : ''}`)
          ...
          if (src.startsWith('data:')) {
            try {
              ...
              if (data.length > 5 * 1024 * 1024) {
                console.warn(`      ⚠️  Data URI too large (${(data.length / 1024 / 1024).toFixed(2)} MB), skipping`)
                continue
              }
              
              // Decode base64
              const imageBuffer = Buffer.from(data, 'base64')
              console.log(`      - Decoded size: ${(imageBuffer.length / 1024).toFixed(2)} KB`)
          ...
          // Handle CID references (Content-ID)
            ...
            if (parsedAttachments) {
              console.log(`      - Searching in ${parsedAttachments.length} parsed attachment(s)...`)
              const attachment = parsedAttachments.find(att => {
                ...
                return normalizedAttCid === normalizedCidFromSrc || 
              ...
              if (attachment && attachment.content) {
                ...
                const filename = attachment.filename || `inline-image-${normalizedCidFromSrc}.${mimeType.split('/')[1] || 'png'}`
                
                console.log(`      ✅ Found matching attachment: ${filename} (${mimeType})`)
                console.log(`      - Attachment size: ${(Buffer.from(attachment.content).length / 1024).toFixed(2)} KB`)
                ...
                console.warn(`      ⚠️  [Extract Images] CID reference not found: ${cidFromSrc} (normalized: ${normalizedCidFromSrc})`)
                if (parsedAttachments.length > 0) {
                  console.log(`      Available CIDs:`, parsedAttachments.map(a => a.cid).filter(Boolean))
                } else {
                  console.log(`      No parsed attachments provided`)
                }
              }
            } else {
              console.warn(`      ⚠️  [Extract Images] CID reference found but no parsedAttachments provided: ${cidFromSrc}`)
            }
          }
        }
      }
  ...
  console.log(`   ✅ [Extract Images] Extraction complete: ${inlineImages.length} image(s) extracted`)
  return inlineImages
}
...
export async function processInlineImages(
  ...
  uploadedImages: Array<{ filename: string; mimeType: string; size: number; fileUrl: string; fileHandle: string }>
}> {
  if (!htmlContent) {
  ...
  inlineImages.forEach((img, idx) => {
    console.log(`   ${idx + 1}. ${img.filename} (${img.mimeType}) - ${(img.data.length / 1024).toFixed(2)} KB`)
    if (img.cid) {
  ...
  const uploadedImages: Array<{ filename: string; mimeType: string; size: number; fileUrl: string; fileHandle: string }> = []
  ...
  for (let i = 0; i < inlineImages.length; i++) {
    ...
    try {
      ...
      if (image.src.startsWith('data:')) {
        ...
        const dataUriPrefix = image.src.substring(0, Math.min(100, image.src.length)) // Use first 100 chars for search
        const tagPattern = /<(img|video)([^>]*?)>/gi
        let match
        ...
        const cidPattern = new RegExp(`(<(?:img|video)([^>]*?)src=["']cid:${cidValue.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(["'][^>]*?)>)`, 'gi')
        processedHtml = processedHtml.replace(cidPattern, (match, p1, p2, p3) => {
          replaced = true
      ...
      if (replaced) {
        console.log(`   ✅ Replaced src in HTML: ${image.src.substring(0, 50)}... -> ${uploadResult.fileUrl}`)
      } else {
        console.warn(`   ⚠️  Could not find and replace src in HTML: ${image.src.substring(0, 50)}...`)
