...
function createImapConnection(config: GmailImapConfig): Imap {
  return new Imap({
    ...
    connTimeout: 180000, // 3 minutes - increased for slow networks
    authTimeout: 120000, // 2 minutes - increased for authentication
    keepalive: {
...
/**
 * Retry wrapper for IMAP operations
 */
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (
        ...
        console.error(`[IMAP] ❌ Authentication failed: ${errorMsg}`);
        console.error(
          '[IMAP] Please check your email address and app password in settings',
        );
        throw error;
      }
...
/**
 * Internal implementation - connects to IMAP and fetches email IDs
 */
async function getEmailIdsInternal(
  config: GmailImapConfig,
  options: FetchOptions,
): Promise<number[]> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          'Connection timeout - failed to connect to Gmail IMAP server',
        ),
      );
    }, 300000); // 5 minutes total timeout for entire operation
    ...
    imap.once('ready', () => {
      ...
      imap.openBox('INBOX', true, (err, box) => {
        ...
        if (options.mode === 'recent') {
          ...
          console.log(
            `[IMAP] Quick sync: checking emails SINCE ${formattedDate} (today)`,
          );
    ...
    imap.once('mail', () => {
      console.log('[IMAP] New mail notification received');
    });

    imap.on('expunge', (seqno) => {
      console.log('[IMAP] Email expunged:', seqno);
    });

    console.log('[IMAP] Attempting to connect to Gmail...');
    imap.connect();
  });
}
...
async function fetchBatchWithNewConnection(
  ...
  return new Promise((resolve, reject) => {
    ...
    console.log(
      `[IMAP] 📦 Batch ${batchNum}/${totalBatches}: Connecting to fetch ${emailIds.length} emails...`,
    );

    const timeout = setTimeout(() => {
      console.log(
        `[IMAP] ⏱️ Batch ${batchNum} timeout, returning ${emailBuffers.size} emails`,
      );
      try {
        imap.end();
      } catch {}
    ...
    imap.once('ready', () => {
      imap.openBox('INBOX', true, (err) => {
        ...
        fetch.once('error', (err) => {
          console.error(
            `[IMAP] ❌ Batch ${batchNum} fetch error: ${err.message}`,
          );
        ...
        fetch.once('end', async () => {
          ...
          clearTimeout(timeout);
          console.log(
            `[IMAP] ✅ Batch ${batchNum}/${totalBatches}: Received ${emailBuffers.size} emails`,
          );
          imap.end();
          resolve(emailBuffers);
        });
    ...
    imap.once('error', (err) => {
      clearTimeout(timeout);
      console.error(
        `[IMAP] ❌ Batch ${batchNum} connection error: ${err.message}`,
      );
...
export async function fetchGmailEmails(
  ...
  console.log(
    `[IMAP] Will fetch ${emailIds.length} emails in ${batches.length} batches of ${BATCH_SIZE}`,
  );
  ...
  async function retryBatch(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          ...
          console.log(
            `[IMAP] Batch ${batchIndex + 1} returned 0 emails, retrying in ${delay / 1000}s (attempt ${attempt + 1}/${maxRetries})`,
          );
        ...
        if (attempt < maxRetries) {
          ...
          console.log(
            `[IMAP] Batch ${batchIndex + 1} failed: ${error.message}, retrying in ${delay / 1000}s (attempt ${attempt + 1}/${maxRetries})`,
          );
          await new Promise((r) => setTimeout(r, delay));
        }
      }
    ...
    console.error(
      `[IMAP] Batch ${batchIndex + 1} failed after ${maxRetries} attempts`,
    );
    return new Map();
  }
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    console.log(
      `[IMAP] Progress: ${allEmailBuffers.size}/${emailIds.length} emails fetched (${Math.round((allEmailBuffers.size / emailIds.length) * 100)}%)`,
    );
  ...
  console.log(
    `[IMAP] All batches complete. Total: ${allEmailBuffers.size}/${emailIds.length} emails`,
  );
  ...
  for (let i = 0; i < entries.length; i += PARSE_BATCH_SIZE) {
    ...
    if (
      (i + PARSE_BATCH_SIZE) % 100 === 0 ||
      i + PARSE_BATCH_SIZE >= entries.length
    ) {
      console.log(
        `[IMAP] ⏳ Parsed ${Math.min(i + PARSE_BATCH_SIZE, entries.length)}/${entries.length} emails`,
      );
    }
  }
 ...
 * IMPORTANT: Never truncates HTML with data URIs - preserves them completely
 */
function truncateContent(
  ...
  // CRITICAL: Never truncate HTML with data URIs - it breaks images
  // Data URIs can be very large but must be preserved intact
  if (content.includes('data:image/') || content.includes('data:video/')) {
    console.log(
      `[IMAP] ⚠️  HTML contains data URIs (${contentBytes} bytes) - skipping truncation to preserve images`,
    );
    return content; // Return full content - don't truncate data URIs
  }

  // For non-image content, truncate safely
  let truncated = content.substring(0, Math.floor(maxSize * 0.9));

  // Try to avoid breaking HTML tags or attributes
  const lastTagEnd = truncated.lastIndexOf('>');
  const lastQuoteEnd = Math.max(
    truncated.lastIndexOf('"'),
    truncated.lastIndexOf("'"),
  );
  const safeCutoff = Math.max(lastTagEnd, lastQuoteEnd);

  if (safeCutoff > maxSize * 0.5) {
    truncated = truncated.substring(0, safeCutoff + 1);
  }
...
> {
  ...
  for (const attachment of attachments) {
    try {
      console.log(
        `[MEGA] Uploading: ${attachment.filename} (${(attachment.size / 1024).toFixed(2)} KB)`,
      );
...
export async function fetchAndStoreGmailEmails(
  ...
  try {
    ...
    console.log(
      `[IMAP] Processing ${fetchedEmails.length} emails for storage...`,
    );
    ...
    const newEmails = fetchedEmails.filter(
      (email) => !existingSet.has(email.messageId),
    );

    // Check existing emails for unprocessed data URIs
    const emailsNeedingReprocessing: Array<{
      email: (typeof fetchedEmails)[0];
      dbEmail: (typeof existingEmails)[0];
    }> = [];
    for (const fetchedEmail of fetchedEmails) {
      const dbEmail = existingEmails.find(
        (e) => e.messageId === fetchedEmail.messageId,
      );
      if (dbEmail) {
        const hasDataUris =
          dbEmail.htmlContent?.includes('data:image/') ||
          dbEmail.htmlContent?.includes('data:video/');
    ...
    console.log(
      `[IMAP] ${existingSet.size} already exist, ${newEmails.length} new`,
    );
    if (emailsNeedingReprocessing.length > 0) {
      console.log(
        `[IMAP] 🔄 ${emailsNeedingReprocessing.length} existing email(s) need reprocessing (have data URIs but no Mega URLs)`,
      );
    }
    ...
    const emailsWithAttachments = newEmails.filter(
      (e) => e.attachments.length > 0,
    );
    const emailsWithoutAttachments = newEmails.filter(
      (e) => e.attachments.length === 0,
    );

    // Increase MAX_CONTENT_SIZE to 2MB to accommodate email bodies with embedded base64 images
    // 60KB was too small and would truncate large inline image data, breaking them
    ...
    if (emailsWithoutAttachments.length > 0) {
      ...
      for (const email of emailsWithoutAttachments) {
        try {
          ...
          const hasDataUris =
            email.htmlContent?.includes('data:image/') ||
            email.htmlContent?.includes('data:video/');
          ...
          if (
            ...
            (hasDataUris ||
              (hasCidRefs &&
                ...
                email.inlineAttachments.length > 0))
          ) {
            try {
              console.log(
                `[IMAP] 🖼️  Processing inline images for email ${email.messageId.substring(0, 20)}...`,
              );
              console.log(
                `[IMAP]    - Data URIs: ${hasDataUris}, CID refs: ${hasCidRefs}, inline attachments: ${email.inlineAttachments?.length || 0}`,
              );
              console.log(
                `[IMAP]    - HTML length: ${email.htmlContent.length} chars`,
              );

              const { processedHtml, uploadedImages } =
                await processInlineImages(
                  email.htmlContent,
                  emailId,
              ...
              if (processedHtml && processedHtml !== email.htmlContent) {
                ...
                console.log(
                  `[IMAP] ✅ Processed ${uploadedImages.length} inline image(s) for email ${email.messageId.substring(0, 20)}`,
                );
                console.log(
                  `[IMAP]    - Processed HTML length: ${processedHtml.length} chars`,
                );
              } else {
                console.log(
                  `[IMAP] ⚠️  No changes after processing inline images`,
                );
              }
            ...
            console.warn(
              `[IMAP] ⚠️ Email ${email.messageId.substring(0, 20)} has CID references but no inline attachments parsed`,
            );
          }

          // Truncate HTML AFTER processing images (to preserve Mega URLs)
          // CRITICAL: Never truncate HTML with images - check original AND processed HTML
          const hasBase64Images =
            processedHtmlContent?.includes('base64,') ?? false;
          const hasDataUriImages =
            processedHtmlContent?.includes('data:image/') ||
            processedHtmlContent?.includes('data:video/');
          // Also check for Mega URLs (images uploaded during processing)
          const hasMegaUrls =
            processedHtmlContent?.includes('/api/storage/mega/') ?? false;
          // Check original HTML - if it had images, don't truncate even if processed
          const originalHadImages =
            email.htmlContent?.includes('data:image/') ||
            email.htmlContent?.includes('data:video/') ||
            email.htmlContent?.includes('cid:');
          const finalHtmlContent =
            hasBase64Images || hasDataUriImages || hasMegaUrls || originalHadImages
              ? processedHtmlContent // Keep full HTML if it has/had images
              : truncateContent(processedHtmlContent, MAX_CONTENT_SIZE);

          // Log if HTML was truncated (indicates loss of content)
          if (
            finalHtmlContent &&
            processedHtmlContent &&
            finalHtmlContent.length < processedHtmlContent.length
          ) {
            console.log(
              `[IMAP] ⚠️  HTML truncated: ${processedHtmlContent.length} -> ${finalHtmlContent.length} chars (max: ${MAX_CONTENT_SIZE})`,
            );
          } else if (hasBase64Images || hasDataUriImages) {
            console.log(
              `[IMAP] ✅ HTML NOT truncated (contains images): ${finalHtmlContent?.length || 0} chars`,
            );
          } else {
            console.log(
              `[IMAP] ✅ HTML size: ${finalHtmlContent?.length || 0} chars (max: ${MAX_CONTENT_SIZE})`,
            );
          }
      ...
      console.log(
        `[IMAP] Stored ${stored} emails (some may have inline images)`,
      );
    }
    ...
    if (emailsWithAttachments.length > 0) {
      console.log(
        `[IMAP] Processing ${emailsWithAttachments.length} emails with attachments...`,
      );
        ...
        try {
          ...
          const upsertedEmail = await prisma.email.upsert({
            ...
            create: {
              ...
              processed: false,
              hasAttachments:
                email.attachments.length > 0 || inlineImagesUploaded > 0,
              createdAt: email.date,
              updatedAt: email.date,
            },
          });
          ...
          const uploadedAttachments = await uploadAttachmentsToMega(
            email.attachments,
            upsertedEmail.id,
          );
    ...
    if (emailsNeedingReprocessing.length > 0) {
      console.log(
        `[IMAP] 🔄 Reprocessing ${emailsNeedingReprocessing.length} existing email(s) with unprocessed images...`,
      );

      for (const {
        email: fetchedEmail,
        dbEmail,
      } of emailsNeedingReprocessing) {
        try {
          console.log(
            `[IMAP] 🔄 Reprocessing email ${fetchedEmail.messageId.substring(0, 20)}...`,
          );

          // Process inline images from the fetched email's HTML
          const hasDataUris =
            fetchedEmail.htmlContent?.includes('data:image/') ||
            fetchedEmail.htmlContent?.includes('data:video/');
          ...
          if (
            ...
            (hasDataUris ||
              (hasCidRefs &&
                ...
                fetchedEmail.inlineAttachments.length > 0))
          ) {
            try {
              const { processedHtml, uploadedImages } =
                await processInlineImages(
              ...
              if (processedHtml && processedHtml !== fetchedEmail.htmlContent) {
                // CRITICAL: Never truncate HTML with images - check original AND processed
                const hasBase64Images =
                  processedHtml?.includes('base64,') ?? false;
                const hasDataUriImages =
                  processedHtml?.includes('data:image/') ||
                  processedHtml?.includes('data:video/');
                // Also check for Mega URLs (images uploaded during processing)
                const hasMegaUrls =
                  processedHtml?.includes('/api/storage/mega/') ?? false;
                // Check original HTML - if it had images, don't truncate
                const originalHadImages =
                  fetchedEmail.htmlContent?.includes('data:image/') ||
                  fetchedEmail.htmlContent?.includes('data:video/') ||
                  fetchedEmail.htmlContent?.includes('cid:');
                const finalHtmlContent =
                  hasBase64Images || hasDataUriImages || hasMegaUrls || originalHadImages
                    ? processedHtml // Keep full HTML if it has/had images
                    : truncateContent(processedHtml, MAX_CONTENT_SIZE);
                ...
                await prisma.email.update({
                  ...
                  data: {
                    ...
                    hasAttachments:
                      (dbEmail.EmailAttachment?.length || 0) +
                ...
                for (const image of uploadedImages) {
                  try {
                    ...
                    console.error(
                      '[IMAP] Error storing reprocessed inline image:',
                      error,
                    );
                  }
                }
                ...
                console.log(
                  `[IMAP] ✅ Reprocessed ${uploadedImages.length} inline image(s) for email ${fetchedEmail.messageId.substring(0, 20)}`,
                );
              }
            } catch (error) {
              console.error(
                `[IMAP] ❌ Error reprocessing inline images for email ${fetchedEmail.messageId.substring(0, 20)}:`,
                error,
              );
            }
          }
          ...
          console.error(
            `[IMAP] ❌ Error reprocessing email ${fetchedEmail.messageId.substring(0, 20)}:`,
            error.message,
          );
          errors++;
        }
    ...
    console.log(
      `[IMAP] ✅ Done: ${stored} stored, ${attachmentsUploaded} attachments, ${errors} errors`,
    );
