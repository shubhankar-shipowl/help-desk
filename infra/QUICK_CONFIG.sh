...
if [ -f "scripts/update-domain.sh" ]; then
  ...
  sed -i '' "s/yourdomain.com/$DOMAIN/g" infra/nginx/conf.d/support-portal.conf 2>/dev/null || true
  sed -i '' "s/yourdomain.com/$DOMAIN/g" docker-compose.prod.yml 2>/dev/null || true
  sed -i '' "s/yourdomain.com/$DOMAIN/g" infra/init-letsencrypt.sh 2>/dev/null || true
fi
echo "✅ Domain updated"

# Step 2: Update email in SSL script
echo "📝 Step 2: Updating email in SSL script..."
sed -i '' "s/your-email@example.com/$EMAIL/g" infra/init-letsencrypt.sh 2>/dev/null || true
echo "✅ Email updated"
...
echo "   ./infra/init-letsencrypt.sh"
